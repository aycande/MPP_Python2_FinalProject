...
#                        ui.input_selectize("year2", "Select Year:", choices = dict(zip(panel3['Year'].unique(), panel3['Year'].unique()))),
                        ui.input_selectize("year2", "Select Year:", choices = dict(zip(panel3['Year'].unique().astype(str), panel3['Year'].unique().astype(str)))),
...
def server(input, output, session):
    ...
    def get_data2():
        ...
        try:
           year = str(pd.to_numeric(input.year2(), errors='raise'))
           print(year)
        except ValueError:
           # Handle the case where input.year2() is not numeric
           print("Input year2() is not numeric.")
    ...
    def cwPlot():
        ...
        ax = sns.scatterplot(x = df[input.x()], y = df[input.y()], hue=df['Country'], marker='o', s=100)
        ax.set_xlabel(f'{input.x()}')
        ax.set_ylabel(f'{input.y()}')
        df_reset = df.reset_index()
        ax.legend(loc='upper left', bbox_to_anchor=(1.05, 1), title='Country')
        for line in range(0, df_reset.shape[0]):
            ax.text(df_reset[input.x()][line], df_reset[input.y()][line], df_reset['Country'].iloc[line],
...
year = 2020
df = df[(df['Year'] == year) & (df['Country'].isin(selected_countries))]
sns.set()
plt.figure(figsize=(12, 8))
df['Year'] = pd.to_numeric(df['Year'], errors='coerce')
plt.legend(loc='upper left', bbox_to_anchor=(1, 1), title='Country')
plt.grid(True, linestyle='--', alpha=0.7)
ax = sns.scatterplot(x = df['LEV2_INT_C_COORD'], y = df['LEV2_INT_C_FIN'], hue=df['Country'], marker='o', linestyle='-')
