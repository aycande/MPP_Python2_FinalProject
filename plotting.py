...
image_path = r'/users/denizaycan/Documents/GitHub/final-project-deniz/images'
...
#First plot : Total colonial emissions vs pledged funding
def plot_non_per_capita_scatter(plot_merge, image_path):
    plt.figure(figsize=(10, 6))
    sns.scatterplot(x='Colonial Total/GtCO2', y='Pledged (USD million current)', data=plot_merge, hue='Country', palette='viridis')
    plt.title('Total Colonial Emissions and Pledges, all countries')
    ...
    plt.tight_layout()
    full_path = os.path.join(image_path, 'noncapita_colonial_funding.png')
    plt.savefig(full_path,  dpi=300)
    plt.show()

plot_non_per_capita_scatter(plot_merge,image_path)
...
#Second plot : Per capita colonial emissions vs pledged funding 
def plot_filtered_scatter(plot_merge, image_path):
    ...
    scatter_plot = sns.scatterplot(x='Colonial consumption per yearly capita /tCO2/pc', y='Pledge per Capita (2023)', data=filtered_data, hue='Country', palette='viridis')

    for i, row in filtered_data.iterrows():
        plt.text(row['Colonial consumption per yearly capita /tCO2/pc'], row['Pledge per Capita (2023)'], row['countrycode'], fontsize=8)

    plt.title('Per capita Colonial Emissions and Pledges, major emitters selected')
    ...
    # Set legend to have two columns
    scatter_plot.legend(bbox_to_anchor=(1.05, 1), loc='upper left', borderaxespad=0., ncol=1)

    plt.tight_layout()
    
    plot_path = os.path.join(image_path, 'percapita_colonial_funding.png')
    plt.savefig(plot_path, dpi=300)
    plt.show()

plot_filtered_scatter(plot_merge, image_path)
...
#Second shiny tab code
def plot_variables(data, x_variable, y_variable, year, countries, image_path):
    ...
    full_path = os.path.join(image_path, f'{x_variable}_{y_variable}_{year}')
...
plot_variables(panel3, x_variable, y_variable, year, selected_countries, image_path)
