...
result = parse_excel_tabs(original_data_path, 'CFU-Website-MASTER-February-2023.xlsx')
if result:
...
UK_ICF = load_andprep_excel_data(original_data_path, 'UK ICF data 2011-2023_read only.xlsx')
...
emissions = load_csv_data(original_data_path, 'ghg-emissions_percapita.csv')
colonial_emissions = load_csv_data(original_data_path, 'full_year_emissions_summary_table_1850_2023.csv')
colonial_for_plot = load_csv_data(original_data_path, 'full_summary_table_2023.csv')
...
#Note that when we download the OECD data from csv instead of API, the data comes with more columns, so we have to drop them first to make it look like what we get it from API.
#Same goes for WB API.
...
def process_gdp_data(df):
    df1 = df.copy()
    df1['countrycode'] = df1['country'].apply(get_country_code)
    ...
    df1['countrycode'] = df1.apply(lambda row: country_code_mapping.get(row['country'], row['countrycode']), axis=1)
...
file_path = os.path.join(final_data_path, 'all_commonwealth_countries.txt')
with open(file_path, 'w') as f:
    for item in all_commonwealth_countries:
        f.write(str(item) + '\n')
ODI_projects_excel_file_path = os.path.join(final_data_path, 'ODI_projects_final.xlsx')
ODI_projects.to_excel(ODI_projects_excel_file_path, index=False)
ODI_projects_cw_excel_file_path = os.path.join(final_data_path, 'ODI_projects_cw_final.xlsx')
ODI_projects_cw.to_excel(ODI_projects_cw_excel_file_path, index=False)
UK_ICF_long_excel_file_path = os.path.join(final_data_path, 'UK_ICF_long_final.xlsx')
UK_ICF_long.to_excel(UK_ICF_long_excel_file_path, index=False)

    #Datasets for plotting & regressions
OECD_excel_file_path = os.path.join(final_data_path, 'OECD_final.xlsx')
OECD.to_excel(OECD_excel_file_path, index=False)
PANEL_excel_file_path = os.path.join(final_data_path, 'panel3_final.xlsx')
panel3.to_excel(PANEL_excel_file_path, index=False)
plot_merge_excel_file_path = os.path.join(final_data_path, 'plot_merge_final.xlsx')
